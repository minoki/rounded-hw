...
sumUnboxedVector' :: forall rn. (Rounding rn) => VU.Vector (Rounded rn Double) -> Rounded rn Double
sumUnboxedVector' (V_Rounded vec) = Rounded (sumUnboxedVector (rounding (Proxy :: Proxy rn)) vec)
{-# INLINE sumUnboxedVector' #-}
